//
//  ICEPhotoBrowserViewController.m
//  DemoPhoto
//
...
#import "ICEZoomScrollView.h"
#import "ICEAssetManager.h"

#define PADDING 10

@interface ICEPhotoBrowserViewController () <UIScrollViewDelegate>
{
    NSMutableSet *_visiblePages, *_recycledPages;   ///< 一个是当前显示的页面，一个是重用页面
}
...
@implementation ICEPhotoBrowserViewController
...
- (void)configUI {
    ...
    if ([self jumpPage] > 0) {
        [self jumpToPageAtIndex:[self jumpPage] animated:NO];
    }
}
...
- (void)jumpToPageAtIndex:(NSUInteger)index animated:(BOOL)animated {
    // Change page
    NSInteger numberOfPhotos = [ICEAssetManager sharedInstance].assetPhotos.count;
...
- (void)onClickSelected:(UIButton *)sender{
    ...
    if (sender.selected) {
        ...
        _btnSelected.transform = CGAffineTransformMakeScale(.5f, .5f);
        [UIView animateWithDuration:.3f
             ...
             usingSpringWithDamping:.5f
              initialSpringVelocity:.5f
                         ...
                         animations:^{
                             _btnSelected.transform = CGAffineTransformIdentity;
                         } completion:nil];
    } else {
...
- (void)updateVisiblePageView{
    ...
    NSInteger pageIndex;
    for (ICEZoomScrollView *page in _visiblePages) {
        pageIndex = page.tag;
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            // Add new page
            ICEZoomScrollView *page = [self dequeueRecycledPage];
            if (page == nil) {
                page = [[ICEZoomScrollView alloc] init];
...
- (ICEZoomScrollView *)dequeueRecycledPage {
    ICEZoomScrollView *page = [_recycledPages anyObject];
    if (page) {
...
- (void)configurePage:(ICEZoomScrollView *)page forIndex:(NSUInteger)index {
    page.tag = index;
...
- (ICEToolBarView *)getToolBarView{
    if (_toolBarView == nil) {
        ...
        _toolBarView = [[ICEToolBarView alloc] initWithBlackColor];
        _toolBarView.frame = frame;
    }
    return _toolBarView;
}
